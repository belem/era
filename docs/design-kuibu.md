...
1. **Apple-inspired minimalism + cultural punctuation** — The app draws from Apple's reductive design philosophy: controlled drama, cinematic spacing, and a singular accent color. Cultural identity comes through typography (KingHwa_OldSong 京華老宋体, Noto Serif SC) and the Living Scroll interaction, not through surface decoration. The design should feel like a precision-crafted instrument, not a museum exhibit.
   ...
   **Voice 1 — Poetry body: KingHwa_OldSong (京華老宋体)**, loaded from `chinese-fonts-cdn.deno.dev`. An elegant Song-style typeface inspired by classical woodblock print lettering. Its structured strokes give poetry a refined, archival quality that pairs naturally with Noto Serif SC headings. Set `font-feature-settings: "kern" 1` and track letter-spacing at 0.05em for poetry lines to give each character room to breathe.
...
| Font System | Three-register stack (see typography detail below) | Poetry: KingHwa_OldSong 京華老宋体. Headings: Noto Serif SC. UI/Pinyin: system sans + Inter |
...
- The aesthetic is clean and minimal with cultural identity expressed through typography (KingHwa_OldSong 京華老宋体, Noto Serif SC), Apple-inspired surfaces, and the blue chop stamp, not surface decoration
...
1. Scaffold Next.js project with Tailwind, configure font stack (KingHwa_OldSong + Noto Serif SC + Inter) and Apple-inspired color tokens (binary surfaces, single blue accent, glass nav)
